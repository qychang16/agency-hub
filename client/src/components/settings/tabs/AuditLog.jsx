  ...
  return (
    <div>
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 20 }}>
        <div>
          <div style={{ fontSize: 15, fontWeight: 600, color: '#14130f' }}>Audit Log</div>
      ...
      {/* Filters */}
      <div style={{ background: '#fff', borderRadius: 10, border: '0.5px solid #dcd8d0', padding: '14px 16px', marginBottom: 16, display: 'flex', gap: 10, flexWrap: 'wrap', alignItems: 'center' }}>
        {/* Search */}
        <div style={{ position: 'relative', flex: 1, minWidth: 200 }}>
          ...
          <input value={search} onChange={e => setSearch(e.target.value)} placeholder="Search by user, action…"
            style={{ width: '100%', padding: '7px 10px 7px 28px', border: '0.5px solid #dcd8d0', borderRadius: 7, fontSize: 12, outline: 'none', background: '#faf9f7', color: '#14130f', boxSizing: 'border-box' }} />
        </div>

        {/* Category filter */}
        <select value={category} onChange={e => setCategory(e.target.value)}
          style={{ padding: '7px 10px', border: '0.5px solid #dcd8d0', borderRadius: 7, fontSize: 12, background: '#faf9f7', color: '#14130f', outline: 'none' }}>
          {FILTER_CATEGORIES.map(c => <option key={c.value} value={c.value}>{c.label}</option>)}
        </select>

        {/* Date range */}
        <div style={{ display: 'flex', alignItems: 'center', gap: 6, background: '#faf9f7', borderRadius: 7, padding: '4px 10px', border: '0.5px solid #dcd8d0' }}>
          <input type="date" value={fromDate} onChange={e => setFromDate(e.target.value)}
            style={{ border: 'none', outline: 'none', fontSize: 11, color: '#4a4742', background: 'transparent' }} />
          <span style={{ fontSize: 11, color: '#9a958c' }}>to</span>
          <input type="date" value={toDate} onChange={e => setToDate(e.target.value)}
            style={{ border: 'none', outline: 'none', fontSize: 11, color: '#4a4742', background: 'transparent' }} />
          {(fromDate || toDate) && (
            <button onClick={() => { setFromDate(''); setToDate('') }} style={{ background: 'transparent', border: 'none', cursor: 'pointer', color: '#9a958c', fontSize: 12, padding: 0 }}>✕</button>
          )}
        </div>

        <span style={{ fontSize: 11, color: '#9a958c', marginLeft: 'auto' }}>{filtered.length} result{filtered.length !== 1 ? 's' : ''}</span>
      </div>
      ...
      {loading ? (
        ...
        <div style={{ background: '#fff', borderRadius: 12, border: '0.5px solid #dcd8d0', overflow: 'hidden' }}>
          <table style={{ width: '100%', borderCollapse: 'collapse' }}>
            ...
            <tbody>
              {filtered.slice(0, 200).map(log => {
                const meta = ACTION_META[log.action] || { label: log.action?.replace(/_/g, ' '), bg: '#f5f3ef', color: '#6e6a63', icon: '📌' }
                const isExpanded = expandedId === log.id
                return [
                  <tr key={log.id} style={{ borderBottom: isExpanded ? 'none' : '0.5px solid #faf9f7', cursor: 'pointer' }}
                    onMouseEnter={e => e.currentTarget.style.background = '#faf9f7'}
                    onMouseLeave={e => e.currentTarget.style.background = isExpanded ? '#faf9f7' : 'transparent'}
                    onClick={() => setExpandedId(isExpanded ? null : log.id)}>
                    <td style={{ padding: '11px 14px', fontSize: 11, color: '#9a958c', whiteSpace: 'nowrap' }}>
                    ...
                    <td style={{ padding: '11px 14px' }}>
                      <div style={{ display: 'flex', alignItems: 'center', gap: 7 }}>
                        <div style={{ width: 26, height: 26, borderRadius: 7, background: '#f5f3ef', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 11, fontWeight: 700, color: '#6e6a63', flexShrink: 0 }}>
                          {(log.user_name || 'S')[0].toUpperCase()}
                        </div>
                        <span style={{ fontSize: 12, fontWeight: 500, color: '#4a4742' }}>{log.user_name || 'System'}</span>
                      </div>
                    </td>
                    <td style={{ padding: '11px 14px' }}>
                    ...
                    <td style={{ padding: '11px 14px', fontSize: 11, color: '#6e6a63' }}>
                      {log.entity_type && <span style={{ textTransform: 'capitalize' }}>{log.entity_type.replace(/_/g, ' ')}</span>}
                      {log.entity_id && <span style={{ color: '#9a958c' }}> #{log.entity_id}</span>}
                    </td>
                    <td style={{ padding: '11px 14px', fontSize: 11, color: '#9a958c', maxWidth: 200 }}>
                      {log.new_values ? (
                  ...
                  isExpanded && (
                    <tr key={`${log.id}-detail`} style={{ borderBottom: '0.5px solid #faf9f7', background: '#faf9f7' }}>
                      <td colSpan={6} style={{ padding: '0 14px 14px 14px' }}>
                        <div style={{ background: '#fff', borderRadius: 8, border: '0.5px solid #dcd8d0', padding: '12px 14px' }}>
                          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: 16, marginBottom: log.old_values || log.new_values ? 12 : 0 }}>
                            <div>
                              <div style={{ fontSize: 10, fontWeight: 600, color: '#9a958c', textTransform: 'uppercase', letterSpacing: '0.4px', marginBottom: 4 }}>Full Timestamp</div>
                              <div style={{ fontSize: 12, color: '#4a4742' }}>{fmtSGT(log.created_at)}</div>
                            </div>
                            <div>
                              <div style={{ fontSize: 10, fontWeight: 600, color: '#9a958c', textTransform: 'uppercase', letterSpacing: '0.4px', marginBottom: 4 }}>Action</div>
                              <div style={{ fontSize: 12, color: '#4a4742' }}>{log.action?.replace(/_/g, ' ')}</div>
                            </div>
                            <div>
                              <div style={{ fontSize: 10, fontWeight: 600, color: '#9a958c', textTransform: 'uppercase', letterSpacing: '0.4px', marginBottom: 4 }}>IP Address</div>
                              <div style={{ fontSize: 12, color: '#4a4742', fontFamily: 'monospace' }}>{log.ip_address || 'Not recorded'}</div>
                            </div>
                          </div>
                            ...
                            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12 }}>
                              {log.old_values && (
                                <div>
                                  ...
                                  <pre style={{ fontSize: 11, color: '#6e6a63', background: '#faf9f7', borderRadius: 6, padding: '8px 10px', margin: 0, overflow: 'auto', fontFamily: 'monospace', lineHeight: 1.5 }}>
                              ...
                              {log.new_values && (
                                <div>
                                  <div style={{ fontSize: 10, fontWeight: 600, color: '#9a958c', textTransform: 'uppercase', letterSpacing: '0.4px', marginBottom: 6 }}>After</div>
                                  <pre style={{ fontSize: 11, color: '#4a4742', background: '#f0fdf4', borderRadius: 6, padding: '8px 10px', margin: 0, overflow: 'auto', fontFamily: 'monospace', lineHeight: 1.5 }}>
          ...
          {filtered.length > 200 && (
            <div style={{ padding: '12px 16px', background: '#faf9f7', borderTop: '0.5px solid #f5f3ef', fontSize: 11, color: '#9a958c', textAlign: 'center' }}>
