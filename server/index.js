...
app.patch('/contacts/:id', auth, async (req, res) => {
  try {
    ...
    const r = await pool.query(`UPDATE contacts SET name=$1, phone=$2, email=$3, type=$4, pipeline_stage=$5, pdpa_consented=$6, dnc=$7, dnc_reason=$8, opted_out=$9, tags=$10, notes=$11, expected_salary=$12, notice_period=$13, linkedin_url=$14, current_role=$15, current_company=$16, updated_at=NOW() WHERE id=$17 AND workspace_id=$18 RETURNING *`, [name, phone, email, type, pipeline_stage, pdpa_consented, dnc, dnc_reason, opted_out, JSON.stringify(tags), notes, expected_salary, notice_period, linkedin_url, current_role, current_company, req.params.id, wsId])
    res.json(r.rows[0])
  } catch (err) { res.status(500).json({ error: err.message }) }
})
