...
async function setupDatabase() {
  ...
  try {
    ...
    await runChunk30InvitationsMigration(pool)
...
app.post('/agents', auth, requirePermission('manage_staff'), async (req, res) => {
  try {
    ...
    if (password) {
      const { valid, errors } = validatePassword(password, cleanEmail)
      if (!valid) {
        return res.status(400).json({
    ...
    const capacityValue = (capacity === '' || capacity === null || capacity === undefined) ? 20 : parseInt(capacity, 10)
    const hash = await bcrypt.hash(finalPassword, 10)
    const forceChange = require_password_change === true

    const r = await pool.query(
      `INSERT INTO users (workspace_id, name, email, password_hash, role, team_id, capacity, force_password_change)
       VALUES ($1, $2, $3, $4, $5, $6, $7, $8) RETURNING *`,
      [wsId, cleanName, cleanEmail, hash, role, teamIdValue, capacityValue, forceChange]
    )
    if (teamIdValue) {
      await pool.query(`INSERT INTO team_members (team_id, user_id) VALUES ($1, $2) ON CONFLICT DO NOTHING`, [teamIdValue, r.rows[0].id])
    }
    await logAudit(wsId, req.user.id, 'create_agent', 'user', r.rows[0].id, null, {
      name: cleanName, email: cleanEmail, role, password_auto_generated: wasAutoGenerated
    })
    const { password_hash, ...safeUser } = r.rows[0]
    res.json({
      ...safeUser,
      initial_password: finalPassword,
      password_was_auto_generated: wasAutoGenerated,
    })
  } catch (err) {
...
app.post('/agents/:id/reset-password', auth, requirePermission('manage_staff'), async (req, res) => {
  try {
    ...
    const targetRes = await pool.query(
      `SELECT id, email, name, is_super_admin FROM users WHERE id = $1 AND workspace_id = $2`,
      [req.params.id, wsId]
    )
    if (!targetRes.rows.length) return res.status(404).json({ error: 'Agent not found' })
    const target = targetRes.rows[0]
    if (target.is_super_admin && target.id !== req.user.id) {
      return res.status(403).json({ error: 'Cannot reset another super admin\'s password' })
    }

    let finalPassword
    let wasAutoGenerated = false
    if (password) {
      const { valid, errors } = validatePassword(password, target.email)
      if (!valid) {
        return res.status(400).json({
          error: 'Password does not meet requirements: ' + errors.join(' '),
          password_errors: errors,
    ...
    const hash = await bcrypt.hash(finalPassword, 10)
    const forceChange = require_change === true

    await pool.query(
      `UPDATE users SET password_hash = $1, force_password_change = $2,
         failed_login_attempts = 0, locked_until = NULL, updated_at = NOW()
       WHERE id = $3 AND workspace_id = $4`,
      [hash, forceChange, target.id, wsId]
    )
    await logAudit(wsId, req.user.id, 'reset_agent_password', 'user', target.id, null, {
      target_email: target.email, password_auto_generated: wasAutoGenerated, force_change: forceChange
    })
    ...
    console.error('POST /agents/:id/reset-password error:', err)
    res.status(500).json({ error: err.message })
  }
})
...
const invitations = createInvitationHandlers(pool, logAudit, getWorkspaceId)
app.post   ('/agents/invite',                          auth, requirePermission('manage_staff'), invitations.inviteAgent)
  app.post   ('/invitations',                            auth, requirePermission('manage_staff'), invitations.inviteAgent)
...
// Self-service profile (chunk 31)
const profile = createProfileHandlers(pool, logAudit)
app.get    ('/me/profile',                  auth, profile.getMyProfile)
